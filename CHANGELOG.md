...
### Changed
- **Stable Typst.ts Runtime** - Upgraded the browser compiler, renderer, and wrapper packages from `0.7.0-rc2` to stable `0.7.0` (Typst 0.14.2)
- **Public Package Imports** - Replaced internal `dist/esm` imports with supported `typst.ts` package entry points
- **Release Validation** - Added automated test, lint, audit, and production build checks for pushes and pull requests

### Security
- **Dependency Patches** - Updated vulnerable transitive sanitizer and build-tool dependencies

### Removed
- **Unused React Wrapper** - Removed the unused `@myriaddreamin/typst.react` dependency
...
### Changed
- **Formula Library UI** - Refined saved formulas layout with a two-column grid and always-visible actions
- **Save Formula Dialog** - Added content preview, auto-name hint, and roomier layout
